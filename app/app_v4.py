...
# Directorio donde están los videos en el servidor.
# Cambia esta ruta a donde tengas tus videos montados/copiados.
...
def scan_videos() -> list[str]:
    """Lista todos los videos en VIDEO_DIR, ordenados por nombre."""
...
def restore_session():
    ...
    S["masks"].clear()
    # NO reconstruir estado SAM2 ni hacer previews aquí.
    # Los puntos son solo datos en RAM. El estado SAM2 se construye
    # limpio una sola vez cuando el usuario presione Propagar.
    ...
    return overlay_frame(S["current"]), (
        ...
        f"Presiona Propagar cuando estés listo"
    )
...
def _rebuild_sam2_state():
    ...
    Llamar SOLO antes de propagar. Libera el estado anterior antes de crear uno nuevo.
    """
    if S["predictor"] is None or S["frame_dir"] is None:
        return
    # Liberar estado anterior antes de crear el nuevo para evitar OOM
    S["inf_state"] = None
    if DEVICE == "cuda":
...
def clear_current_frame():
    ...
    _save_points()
    # No reconstruir estado SAM2 aquí — se hará limpio antes de propagar.
    # Reconstruirlo ahora re-cargaría todos los frames a GPU (OOM).
...
def clear_all():
    ...
    S["masks"].clear()
    _save_points()
    # No llamar init_state aquí — re-cargaría todos los frames a GPU (OOM).
    # El estado SAM2 se reconstruye limpio en _rebuild_sam2_state() antes de propagar.
...
def release_app():
    """Libera el bloqueo. El redirect a /logout lo hace el JS después."""
    _release_lock()
    return ""
...
with gr.Blocks(
            ...
            with gr.Group():
                ...
                with gr.Row():
                    video_dd    = gr.Dropdown(
                        choices=scan_videos(),
                        value=None,
                        label=f"Videos en {VIDEO_DIR}/",
                        show_label=True,
                        scale=4,
                    )
                    refresh_btn = gr.Button("🔄", scale=1, size="sm", min_width=40)
                with gr.Row():
    ...
    load_vid_btn.click(load_video, inputs=[video_dd, stride_sl, maxdim_sl], outputs=[canvas, vid_status])
