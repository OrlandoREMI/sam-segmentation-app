...
def _rebuild_sam2_state():
    ...
    if S["predictor"] is None or S["frame_dir"] is None:
        return
    _free_inf_state()
    S["inf_state"] = S["predictor"].init_state(video_path=S["frame_dir"], offload_video_to_cpu=True)
...
def load_video(video_name: str, stride: int = 2, max_dim: int = 1280):
    ...
    if S["predictor"] is not None:
        S["inf_state"] = S["predictor"].init_state(video_path=str(frame_dir), offload_video_to_cpu=True)
